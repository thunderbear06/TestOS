...
EFI_STATUS efi_main (EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
	...
	if (Kernel == NULL) {
		Print(L"FAILED TO LOAD KERNEL! Now what did you do? \n\r");
	}
	else {
		Print(L"Kernel has been loaded successfully \n\r");
	}
	...
	if (newFont == NULL){
		Print(L"Failed to load font, either font is not valid or can't be found!\n\r", newFont->psf1_Header->charsize);
	}
	else
	{
		Print(L"Font loaded successfully! char size = %d\n\r");
	}
	...
	Print(L"Base: 0x%x\n\rSize: 0x%x\n\rWidth: %d\n\rHeight: %d\n\rPixelsPerScanLine: %d\n\r", 
		...
		newBuffer->BufferSize, newBuffer->Width, 
		newBuffer->Height, 
